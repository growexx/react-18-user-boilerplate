...
describe('injectors', () => {
  ...
  describe('ejectSaga helper', () => {
    ...
    it('should cancel a saga in RESTART_ON_REMOUNT mode', () => {
      ...
      globalStore.injectedSagas.test = {
        task: { cancel },
    ...
    it('should not cancel a daemon saga', () => {
      const cancel = jest.fn();
      globalStore.injectedSagas.test = { task: { cancel }, mode: DAEMON };
      ejectSaga('test');
    ...
    it("should remove non daemon saga's descriptor in production", () => {
      ...
      expect(globalStore.injectedSagas.test).toBe('done');
      expect(globalStore.injectedSagas.test1).toBe('done');
    ...
    it("should not remove daemon saga's descriptor in production", () => {
      ...
      expect(globalStore.injectedSagas.test.saga).toBe(testSaga);
      process.env.NODE_ENV = originalNodeEnv;
    });
  ...
  describe('injectSaga helper', () => {
    ...
    it('should pass args to saga.run', () => {
      const args = {};
      globalStore.runSaga = jest.fn();
      injectSaga('test', { saga: testSaga }, args);

      expect(globalStore.runSaga).toHaveBeenCalledWith(testSaga, args);
    });
    ...
    it('should not start daemon and once-till-unmount sagas if were started before', () => {
      ...
      expect(globalStore.runSaga).toHaveBeenCalledTimes(2);
    });
    ...
    it('should start any saga that was not started before', () => {
      ...
      expect(globalStore.runSaga).toHaveBeenCalledTimes(3);
    });

    it('should restart a saga if different implementation for hot reloading', () => {
      const cancel = jest.fn();
      globalStore.injectedSagas.test = { saga: testSaga, task: { cancel } };
      globalStore.runSaga = jest.fn();
      ...
      expect(globalStore.runSaga).toHaveBeenCalledWith(testSaga1, undefined);
    });
    ...
    it('should save an entire descriptor in the saga registry', () => {
      ...
      expect(globalStore.injectedSagas.test.foo).toBe('bar');
    });
  });
});
